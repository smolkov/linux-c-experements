...
static int delete_semid = 0;
static int delete_shmid = 0;
static void locksem(int semid, int semnum) {
  struct sembuf sb;
  sb.sem_num = semnum;
  sb.sem_op = -1;
...
static int safesemget(key_t key, int nsems, int semflg) {
  ...
  if (retval == -1)
    printf(
        "Semaphor-Schlüssel %d, nsems %d konnte"
...
static int safesemctl(int semid, int semnum, int cmd, union semun arg) {
  ...
  if (retval == -1)
    printf(
        "Fehler: Semaphor mit ID %d, semnum %d, "
        "Kommando %d\n",
        semid, semnum, cmd);
  return retval;
}
...
static sighandler_t my_signal(int sig_nr, sighandler_t signalhandler) {
  ...
  if (sigaction(sig_nr, &neu_sig, &alt_sig) < 0) return SIG_ERR;
